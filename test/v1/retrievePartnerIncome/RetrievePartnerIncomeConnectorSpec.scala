...
class RetrievePartnerIncomeConnectorSpec extends ConnectorSpec {

  "retrievePartnerIncome" should {
    "return a success response" in new HipTest with Test {
      val expected: Right[Nothing, ResponseWrapper[RetrievePartnerIncomeResponse]] = Right(ResponseWrapper(correlationId, responseModel))

      willGet(url = url"$baseUrl/itsa/income-tax/v1/26-27/income/partnerships/$nino?partnershipUTR=$partnershipUtr")
        .returning(Future.successful(expected))

      val result: DownstreamOutcome[RetrievePartnerIncomeResponse] = await(connector.retrievePartnerIncome(request))
  ...
  trait Test {
    self: ConnectorTest =>
    val connector: RetrievePartnerIncomeConnector = new RetrievePartnerIncomeConnector(http = mockHttpClient, appConfig = mockAppConfig)
  }
