...
import api.models.domain.{PartnershipUtr, TaxYear}
...
class DeletePartnerIncomeControllerSpec
    extends ControllerBaseSpec
    with ControllerTestRunner
    with MockAppConfig
    with MockDeletePartnerIncomeService
    with MockDeletePartnerIncomeValidatorFactory
    with MockAuditService {

  private val partnershipUtr = "1234567890"
  ...
  "deletePartnerIncome" should {
    ...
    "return the error as per spec" when {
      "the parser validation fails" in new Test {
        ...
        runErrorTestWithAudit(PartnershipUtrFormatError)
      }
  ...
  private trait Test extends ControllerTest with AuditEventChecking[GenericAuditDetail] {
    ...
    val controller: DeletePartnerIncomeController = new DeletePartnerIncomeController(
      authService = mockEnrolmentsAuthService,
      lookupService = mockMtdIdLookupService,
      service = mockDeletePartnerIncomeService,
      validatorFactory = mockDeletePartnerIncomeValidatorFactory,
    ...
    protected def event(auditResponse: AuditResponse, maybeRequestBody: Option[JsValue]): AuditEvent[GenericAuditDetail] =
      AuditEvent(
        ...
        detail = GenericAuditDetail(
          ...
          params = Map("nino" -> validNino, "taxYear" -> taxYear, "partnershipUtr" -> partnershipUtr),
